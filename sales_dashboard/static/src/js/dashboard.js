...
class ChartJSRenderer extends owl.Component {
  ...
  renderChart() {
    ...
    this.chartJS = new Chart(this.chartRef.el, {
  ...
  renderAmChart() {
    ...
    switch (chartType) {
      case "column-chart":
        ...
        function makeSeries(name, fieldName, root) {
        ...
        makeSeries("Quotations", "quotation", this.root);
        makeSeries("Orders", "order", this.root);
        ...
        series2.bullets.push(() => {
          return am5.Bullet.new(this.root, {
            ...
            sprite: am5.Circle.new(this.root, {
              ...
              strokeWidth: 2,
              fill: this.root.interfaceColors.set("fill", am5.color("#D5ED9F")),
            }),
          });
        });
...
class Dashboard extends owl.Component {
      ...
      datasets: [
        {
          label: "Qoutations",
          data: data
            .filter((d) => d.state == "draft" || d.state == "sent")
            .map((d) => d.price_total),
        },
        {
          label: "Orders",
          data: data
            .filter((d) => ["sale", "done"].includes(d.state))
            .map((d) => d.price_total),
        },
      ],
    };
  ...
  async getOrders() {
    ...
    const rev_percentage =
      ((current_revenue[0].amount_total - prev_revenue[0].amount_total) /
        prev_revenue[0].amount_total) *
      100;

    // revenue order
    const current_avg = await this.orm.readGroup(
      "sale.order",
      domain,
      ["amount_total:avg"],
      []
    );
    const prev_avg = await this.orm.readGroup(
      "sale.order",
      prev_domain,
      ["amount_total:avg"],
      []
    ...
    const avg_percentage =
      ((current_avg[0].amount_total - prev_avg[0].amount_total) /
        prev_avg[0].amount_total) *
      100;
